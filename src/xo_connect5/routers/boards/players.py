...
from xo_connect5.models.users import Order, OrderType, Player, Players
...
def get_player_from_order_in_board(order: Order, board: Board) -> Optional[Player]:
    sitting_player_dict = board.players.dict().get(order.type)
...
async def join_player(param: PlayersParam = Depends()) -> Players:
    ...
    board.players = board.players.copy(update={order.type: join_player}, deep=True)
    return board.players
